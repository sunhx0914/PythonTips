...
实现在 WSGI_1 文件夹下，上面为centos中，win10需要修改细节
...
# 定义一个函数
def test(number):
    # 在函数内部再定义一个函数，并且这个函数用到外边函数的变量，那么将这个函数以及用到的一些变量称之为闭包
...
这个例子中，函数line与变量a,b构成闭包。在创建闭包的时候，通过line_conf的参数a,b说明了这两个变量的取值，这样，就确定了函数的最终形式(y = x + 1和y = 4x + 5)。只需要变换参数a,b，就可以获得不同的直线表达函数。由此，闭包也具有提高代码可复用性的作用。

如果没有闭包，需要每次创建直线函数的时候同时说明a,b,x。这样，就需要更多的参数传递，也减少了代码的可移植性。

注意点：由于闭包引用了外部函数的局部变量，则外部函数的局部变量没有及时释放，消耗内存。与创建实例对象的方式相比，没有一些类中的魔法方法，减少内存消耗。
...
![QQ20171104-092312@2x](D:\BaiduNetdiskDownload\基础班-就业班课件资料 -1-4位基础班  5-14就业班\05python和linux高级编程阶段\1-6代码和截图\17-闭包和装饰器\QQ20171104-092312@2x.png)

> 闭包可以理解为内存中的一块空间，包含数据和函数
...
类似于修改全局变量的方式( `global`)，闭包中使用 `nonlocal` 关键字

##### 装饰器

装饰器可以在装饰的函数 **之前** 或者 **之后** 添加功能。

> **函数名**仅仅是个**变量**，只不过指向了定义的函数而已，所以才能通过 函数名()调用，如果 函数名=xxx被修改了，那么当在执行 函数名()时，调用的就不是之前的那个函数了。

> 写代码要遵循 **开放封闭** 原则，虽然在这个原则是用的面向对象开发，但是也适用于函数式编程，简单来说，它规定已经实现的功能代码不允许被修改，但可以被扩展，即：
>
> - 封闭：已实现的功能代码块。**对修改封闭**，意味着类一旦设计完成，就可以独立完成其工作，而不要对类进行任何修改。
> - 开放：对扩展开放。**对扩展开放**，意味着有新的需求或变化时，可以对现有代码进行扩展，以适应新的情况。
...
# @函数名 是python的一种语法糖
@set_func  # 等价于test1 = set_func(test1) ，将原来函数的引用test1当作实参传递给闭包
def test1():
    print("-----test1----")

# ret = set_func(test1)  # 通过 @set_func 方式实现
# ret()

test1()
```

![QQ20171104-112615@2x](https://i.loli.net/2020/05/07/gnGo9JkKC2AIEHZ.png)

> `test1` 指向了闭包，闭包中的 `func` 指向了原来的函数
...
# test1 先作为参数赋值给func后, test1 接收指向 set_func 返回的 call_func
test1()
# 调用 test1(),即等价调用 call_func()
# 内部函数 call_func 被引用，所以外部函数的 func 变量(自由变量)并没有释放
# func里保存的是原 test1 函数对象"""
# 2. 对有参数、无返回值的函数进行装饰
def set_func(func):
    def call_func(a):
        print("---这是权限验证1----")
...
def set_func(func):
    print("---开始进行装饰")
    def call_func(*args, **kwargs):  # 这不需要num  num属于 *args, 第一个值给num
        print("---这是权限验证2----")
...
def set_func(func):
    ...
    def call_func(*args, **kwargs):
        print("---这是权限验证1----")
        # func(args, kwargs)  # 不行，相当于传递了2个参数 ：1个元组，1个字典
        return func(*args, **kwargs)  # 拆包  只需要将返回的结果再次返回就可以了
    return call_func
@set_func  # 相当于 test1 = set_func(test1)
def test1(num, *args, **kwargs):
    print("-----test1----%d" % num)
...
ret = test1(100)
print(ret)
ret = test2()  # 无返回值 则为 None
print(ret)
""" 通用装饰器：接收不定长，拆包传参数，调用原函数后返回 """
# 5. 一个装饰器可以对多个函数进行装饰，并且装饰器在调用函数之前，已经被python解释器执行了
def set_func(func):
    print("---开始进行装饰")
    def call_func(a):
        print("---这是权限验证1----")
...
def test1(num):
    print("-----test1----%d" % num)
@set_func  # 一个装饰器对多个函数进行装饰，会创建多个闭包  每装饰一次，一个闭包
def test2(num):
    print("-----test2----%d" % num)
# 装饰器在调用函数之前，已经被python解释器执行了，所以 当调用函数之前 其实已经装饰好了，尽管调用就可以了
# test1(100)
# test2(200)
"""运行结果：
---开始进行装饰
...
def add_qx(func):
    print("---开始进行装饰权限1的功能---")
    def call_func(*args, **kwargs):  # 通用写法：接收不定长参数
        print("---这是权限验证1----")
        return func(*args, **kwargs)  # 拆包传参数，调用原函数后返回
    return call_func
def add_xx(func):
    print("---开始进行装饰xxx的功能---")
    def call_func(*args, **kwargs):
...
def test1():
    print("------test1------")
test1()
"""先装下面的，后装上面的  下面是函数才可以装, 所以后面的先装
执行的时候正常 上到下
"""
```

![QQ20171104-121431@2x](https://i.loli.net/2020/05/07/7cIFHQpO859dJbW.png)
...
class Test(object):
    def __init__(self, func):
        self.func = func
    def __call__(self):  # 在这加参数 通用的那种
        print("这里是装饰器添加的功能.....")
        return self.func()

@Test  # 相当于get_str = Test(get_str)  这 @Test.静态方法名 等都可以做装饰器
def get_str():
    return "haha"

print(get_str())
#说明：
#1. 当用Test来装作装饰器对get_str函数进行装饰的时候，首先会创建Test的实例对象
#   并且会把get_str这个函数名当做参数传递到__init__方法中
#   即在__init__方法中的属性func指向了get_str指向的函数
#
#2. get_str指向了用Test创建出来的实例对象
#
#3. 当在使用get_str()进行调用时，就相当于让这个对象()，因此会调用这个对象的__call__方法
#
#4. 为了能够在__call__方法中调用原来get_str指向的函数体，所以在__init__方法中就需要一个实例属性来保存这个函数体的引用
#   所以才有了self.func = func这句代码，从而在调用__call__方法中能够调用到get_str之前的函数体
```

###### 装饰器带参数

```python
# 方法：在原有装饰器的基础上，设置外部变量  实现方式：再嵌套一层函数
...
```python
# 使用装饰器自动填充字典
# 由于 装饰器在函数调用前已经完成了装饰，所以可行
# 字典中的index指向的原来的函数（装饰器传入时参数，其指向原函数）  而原来函数名index指向了闭包
# 用不到index指向闭包，闭包指向原函数 的过程  装饰器里面没用到 -- 装饰器目前只是为了添加字典
import re
"""用来存放url路由映射
URL_FUNC_DICT = {
    "/index.py": index,
...
def route(url):
    def set_func(func):
        ...
        def call_func(*args, **kwargs):  # 这个不会执行
            return func(*args, **kwargs)
        return call_func
    return set_func
...
def application(env, start_response):
    ...
    # file_name = "/index.py"
    """
    if file_name == "/index.py":  # 去掉 if  根据请求，自动调
        return index()
        ...
        return 'Hello World! 我爱你中国....'
    """
    try:  # 使用异常是因为 异常可以传递，可以获取调用函数中出现的异常
        # func = URL_FUNC_DICT[file_name]
        # return func()
        return URL_FUNC_DICT[file_name]()  # 这执行的字典中指向的函数
    except Exception as ret:
        return "产生了异常：%s" % str(ret)
    # 如果 有 index() ,才会执行装饰器的内容
```

![QQ20171105-101342@2x](https://i.loli.net/2020/05/07/x1rUpBfVjA7kvNt.png)

只使用黄色部分，通过装饰器实现自动添加字典，而没有使用红色的装饰器的部分
...
- 缺点是：最大的缺点是如果是中大型网站，则产生的页面特别多，不好管理。
...
- 一句话总结:对SEO的影响：和动态URL一样，对SEO没有什么减分影响。

##### 实现伪静态url

需要修改 `web_server.py`文件、模板文件中的超链接、框架中的装饰器参数
...
> 这的页面代码是前端写好的，包括跳转超链接地址等，前端页面中有部分示例数据，具有格式，后端使用这些格式替换数据
>
> 后端写接口，用返回的数据替换前端页面中的示例数据

全部实现在 WSGI_2_route_mysql
...
```python
def index(ret):  # 多了一个参数，需要修改下
def center(ret):  # 这两个里面 点击链接部分原来是写的固定值，也需改为动态的
# ...
@route(r"/add/(\d+)\.html")
def add_focus(ret):
    # 1. 获取股票代码
    stock_code = ret.group(1)  # 通过匹配的结果 获取股票代码
    # 2. 判断试下是否有这个股票代码  防止自己构造不存在代码访问 /add/88888.html
    conn = connect(host='localhost',port=3306,user='root',
                   password='mysql',database='stock_db',charset='utf8')
    cs = conn.cursor()
...
# ...
if ret:
    return func(ret)  # 将匹配的结果作为实参传入  虽然 index center 不会用到
```

>注意：
>
>1. 页面的内容是前端写好的，如对应的超链接地址等，后端按格式返回数据就行
>2. 框架写的比较全面的话，**添加功能**的话就是再多**增加一个函数**就行，如上面的add_focus函数，其余部分都是不需要再修改的
...
def del_focus(ret):
    ...
    sql = """ select * from info as i inner join focus as f on i.id=f.info_id where i.code=%s;"""
    cs.execute(sql, (stock_code,))
    # 如果没有关注过，那么表示非法的请求  没有关注的话无需取消关注
    if not cs.fetchone():
        cs.close()
        conn.close()
...
def show_update_page(ret):
    ...
    with open("./templates/update.html") as f:
        content = f.read()
    # 默认存在，可以修改  可以向上面一样，补全情况
    # 3. 根据股票代码查询相关的备注信息  之前可以判断有无
    conn = connect(host='localhost',port=3306,user='root',
                   password='mysql',database='stock_db',charset='utf8')
    cs = conn.cursor()
    ...
    return content
@route(r"/update/(\d+)/(.*)\.html")  # 这页面的地址 是前端中写好的
def save_update_page(ret):  # 保存更新前应该判断 有无股票 内容有无sql注入  有无病毒 等
    """"保存修改的信息"""
    stock_code = ret.group(1)
    comment = ret.group(2)
    conn = connect(host='localhost',port=3306,user='root',
...
浏览器会将中文、空格等自动编码，，因此框架在数据保存前需要解码

程序中使用`GET`方式传输备注信息    备注信息中多个空格时，浏览器只会显示一个空格，数据库中还是多个
...
# 开始使用log功能
logging.info('这是 loggging info message')  # 类似于print 但带有级别
...
https://www.cnblogs.com/xiangxiaolin/p/11655741.html



通过协议解耦   功能分开  http WSGI 等

#### 元类

元类    一种特殊的类 用于创建类

**给类加功能可以用元类**，类似于装饰器给函数加功能

# 内建模块/内嵌模块  globals()、内嵌函数  `__builtin__模块默认加载，`

调用的时候先在 globals() 中找，没有去 `__builtin__` 中找

globals() 返回值为字典 有什么 可以用什么    全局变量



##### 类也是对象

类就是一组用来描述如何生成一个对象的代码段。在Python中，**类也是一种对象**。
...
# <class '__main__.ObjectCreator'>
print(hasattr(ObjectCreator, 'new_attribute'))  # hasattr() 函数用于判断对象是否包含对应的属性。
# Fasle
...
可通过函数中的if判断，根据传入的不同，创建不同的类，实现动态创建，可以实现但不是好办法。
...
class Test: #定义了一个Test类
    pass
Test2 = type("Test2", (), {}) # 定了一个Test2类  返回值是一个类对象  type是类
print(Test())  # 创建了一个Test类的实例对象
print(Test2())  # 创建了一个Test2类的实例对象
print(help(Test)) # 用help查看Test类
print(help(Test2)) # 用help查看Test2类
```
...
`FooChild = type('FooChild', (Foo,), {})` 继承Foo类
...
def print_b(self):  # 用于添加实例方法
    print(self.num)

@staticmethod  # 用于添加静态方法
def print_static():
    print("----haha-----")
...
def print_class(cls):
    print(cls.num)
# bar 为类属性
B = type("B", (A,), {"bar": True, "print_b": print_b, "print_static": print_static, "print_class": print_class})
b = B()
b.print_b()
b.print_static()
b.print_class()
...
创建类就是为了创建类的实例对象，元类就是用来创建这些类（对象）的
...
> str是用来创建字符串对象的类，而int是用来创建整数对象的类。type就是创建类对象的类。你可以通过检查`__class__`属性来看到这一点。Python中所有的东西，注意，我是指所有的东西——**都是对象**。这包括整数、字符串、函数以及类。它们全部都是对象，而且它们都是从一个类创建而来，这个类就是type。
...
def foo(): pass
print(foo.__class__, foo.__class__.__class__)
class Bar(object): pass
print(Bar.__class__, Bar.__class__.__class__)
```
...
```python
class Foo(object):  # (object, metaclass=something)  python3
    __metaclass__ = something…  # python2
    ...省略...
```
...
4. 如果还是找不到`__metaclass__`,Python就会用内置的type来创建这个类对象。(原来定义类的时候都是type)
...
```python
#-*- coding:utf-8 -*-
def upper_attr(class_name, class_parents, class_attr):  # 这传入 type 的参数 
    # 'Foo'  (object,)  {"bar":"bip"}
    
    #遍历属性字典，把不是__开头的属性名字变为大写  还可以默认继承 及修改私有属性的名字
    new_attr = {}
    for name,value in class_attr.items():
        if not name.startswith("__"):  # 过滤 object 中的
            new_attr[name.upper()] = value
    #调用type来创建一个类
    return type(class_name, class_parents, new_attr)

class Foo(object, metaclass=upper_attr):  # python3
    bar = 'bip'

print(hasattr(Foo, 'bar'))
print(hasattr(Foo, 'BAR'))

f = Foo()  # Foo 指向了新的type创建的
print(f.BAR)
```

再做一次，这一次用一个真正的class来当做元类。

```python
class UpperAttrMetaClass(type):  # 继承type 认为是元类
...
一个句话理解就是：创建一个实例对象，用创建它的**类名**当做**数据表名**，用创建它的**类属性**对应数据表的**字段**，当对这个实例对象操作时，能够对应MySQL语句
...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__mappings__'] = mappings  # 保存属性和列的映射关系  使其不出现在类属性中
        attrs['__table__'] = name  # 假设表名和类名一致
        return type.__new__(cls, name, bases, attrs)
    
class User(metaclass=ModelMetaclass):
    uid = ('uid', "int unsigned")  # 元组
    name = ('username', "varchar(30)")
    email = ('email', "varchar(30)")
    password = ('password', "varchar(30)")
    ...
    def __init__(self, **kwargs):
        for name, value in kwargs.items():
            setattr(self, name, value)  # 这self.不可以 self.name存为name，不会取name的值 不是uid等了
            # 而且 self.name 只会存最后的值
            # self 指向一个对象 对象的属性有 uid=12345 ...
    def save(self):  # 转换为 sql语句
        fields = []  # ["uid", "username"...]
        args = []  # [12345, ...]
        for k, v in self.__mappings__.items():
            fields.append(v[0])
            args.append(getattr(self, k, None))
        sql = 'insert into %s (%s) values (%s)' % (self.__table__, ','.join(fields), ','.join([str(i) for i in args]))  # 这的数据没有 双引号，需要加
        print('SQL: %s' % sql)
...
class Model(object, metaclass=ModelMetaclass):  # 这样可以重用了
    def __init__(self, **kwargs):
        for name, value in kwargs.items():
            setattr(self, name, value)  # 这self. 不可以    self.name 存为name了
    def save(self):
        fields = []
        args = []
        for k, v in self.__mappings__.items():
        ...
        for temp in args:
            ...
            if isinstance(temp, int):
                args_temp.append(str(temp))  # ["12345"]
            elif isinstance(temp, str):
                args_temp.append("""'%s'""" % temp)  # 这没有 """ """ 的话，sql语句中就没有引号了
        sql = 'insert into %s (%s) values (%s)' % (self.__table__, ','.join(fields), ','.join(args_temp))  # join 会去掉最外面一层引号
        print('SQL: %s' % sql)
        
class User(Model):
    uid = ('uid', "int unsigned")  # Django中这里不是元组 而是实例对象
    name = ('username', "varchar(30)")
    email = ('email', "varchar(30)")
    password = ('password', "varchar(30)")
...
普通定义类时 就是在 class 处 用 type 创建类了
...
多任务-线程---------------->重点：进程、线程、协程 的区别，互斥锁 解决线程资源竞争 缺点产生死锁，GIL   互斥锁与GIL:互在逻辑上保证一个可能很长的事情做完，G在调度角度只能保证同一时刻只有一个线程执行，不见得把事情做完了，谁执行，执行多少次有默认策略     计算密集型用进程  线程在默认解释器上是假的   协程利用线程堵塞/消耗时间的地方  协程是假的多任务   多进程+协程
	多任务的介绍
	多任务的概念
	线程(重点)
	线程-注意点
...
http协议、web服务器-并发服务器1---------------->重点：HTTP协议, 访问baidu.com的过程， 浏览器和服务器之间的交互  访问baidu.com的过程中域名解析是重点   3次握手  网关  mac变 ip不变  
	http协议
	Web静态服务器-1-显示固定的页面
	Web静态服务器-2-显示需要的页面
	Web静态服务器-3-多进程
...
WSGI、mini-web框架---------------->重点：WSGI 协议规定  服务器如何把数据给框架 解耦
	服务器动态资源请求
	应用程序示例
	Web动态服务器-基本实现
	mini-web框架-1-文件结构
...
https://zhuanlan.zhihu.com/p/24283040  Python进阶：聊聊IO密集型任务、计算密集型任务，以及多线程、多进程

https://blog.csdn.net/fervor_heart/article/details/12708313 为什么 tcp/udp 的端口号可以重复

https://blog.csdn.net/wm12345645/article/details/82381407 面试题：进程间通信的方式  socket只是进程中通信的一种方式 这其他方式深入C语言会涉及

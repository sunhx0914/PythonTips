...
进程：一个程序运行起来后，**代码 +** 用到的**资源** 称之为进程，它是操作系统分配资源的基本单元。
...
VScode 运行时，主程序的父进程id为执行程序的终端的进程号，主进程和子进程都为 Python 解释器

> 强制结束一个进程时，其他进程继续运行，强制结束所有进程后，**命令行阻塞(什么状态？等待？)**；但如何主进程已经运行结束(非无限循环)，强制结束子进程后，代码运行终止。

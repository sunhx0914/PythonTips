...
![三次握手、四次挥手](https://user-images.githubusercontent.com/51505633/79301086-28963a80-7f1b-11ea-81e8-874a05acc295.png)
...
普通的方式，单进程-单线程会堵塞，导致多浏览器访问时等待，所以使用多线程、多进程为其服务；上边是单进程-单线程，通过非阻塞监听多个套接字，实现了同样的目的。

因为列表的循环执行的，所以是并发服务器。

电脑会先收数据，有个缓存区，recv 时取走数据(0.5 秒检查，短时间到达缓存区的，会一起取走)，没有产生异常/堵塞。
...
因为互联网协议包含了上百种协议标准，但是最重要的两个协议是TCP和IP协议，所以，大家把互联网的协议**简称TCP/IP协议(族)**
...
tcp    ip.src   ip.dst    udp.port == 2405

ip.src == 192.168.33.45 and(or) udp
...
网络掩码   与 ip  按位与操作   都1 为1  

确定 主机号  网络号   确定是否为同一网络

网卡要先判断  需要 mac地址   arp 

网卡默认接收  FF:FF:FF:FF:FF:FF
...
路由器 同一台电脑 两个网卡    ip地址不变  源、目的mac在变
